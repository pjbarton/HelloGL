...
int Model_PLY::Load(char* filename)
{
	...
	if (file)
	{
		...
		for (int iterator = 0; iterator < this->TotalConnectedPoints; iterator++)
		{
			...
			Vertex_Buffer[i] *= myScale;
			Vertex_Buffer[i+1] *= myScale;
			Vertex_Buffer[i+2] *= myScale;
			//printf("%f %f %f\n", Vertex_Buffer[i], Vertex_Buffer[i + 1], Vertex_Buffer[i + 2]);
			i += 3;
		}
